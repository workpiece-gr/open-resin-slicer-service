...
`immutable STL -> [oriented PrusaSlicer review geometry (.3mf) + merged effective SLA config (.ini)] -> PrusaSlicer SLA slice (.sl1) -> UVtools conversion/inspection -> printer-native .ctb/.goo`

The **review 3MF and effective SLA config are created first in the same pinned PrusaSlicer invocation**. The selected machine, resin and quality profiles are merged by PrusaSlicer and saved as the retained effective `.ini`; the requested orientation is applied to the geometry exported to the review `.3mf`.

This distinction is intentional. In the pinned PrusaSlicer CLI path, `--export-3mf` exports model/review geometry without passing the print configuration to the 3MF writer, so Workpiece does **not** treat that file as a self-contained Prusa project recipe. The SLA slice is generated by loading the exact retained effective config together with the exact retained review 3MF, without reapplying the requested rotations. The 3MF SHA-256 and effective-config SHA-256 therefore jointly bind the retained slicing recipe.

The API returns a review ZIP containing:

- immutable source STL;
- oriented PrusaSlicer review `.3mf`;
- merged effective PrusaSlicer SLA `.ini`;
- intermediate `.sl1`;
- exact printer-native `.ctb`/`.goo`;
- `manifest.json` with hashes, engines, profile IDs, orientation and recipe roles;
- raw UVtools issue report.

The intended workshop flow mirrors Workpiece FDM authority as closely as resin formats allow:

1. inspect the retained 3MF for the exact oriented geometry and inspect the retained effective `.ini` for the exact SLA settings used by the pinned engine;
