...
The implemented server-side library path instead separates orientation into independently testable stages:
...
A proxy score never grants manufacturing authority. If all sliced finalists are blocked, the result requires manual review rather than silently choosing one.
...
A `production-authoritative` selected order requires one matching plate-authority object for every physical plate. The order binds its retained 3MF, SL1, native hashes and issue receipt back to those authority objects.
